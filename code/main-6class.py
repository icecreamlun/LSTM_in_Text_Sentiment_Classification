...
vocab_path = './data6/vocab.pkl'             # Vocabulary file
save_path = './saved_dict/lstm.ckpt'        # Model training results
embedding_pretrained = \
    torch.tensor(
    np.load(
        './data6/embedding_random.npz')
...
def get_data():
    ...
    train = load_dataset('./data6/train.tsv', pad_size, tokenizer, vocab)
    dev = load_dataset('./data6/dev.tsv', pad_size, tokenizer, vocab)
    test = load_dataset('./data6/test.tsv', pad_size, tokenizer, vocab)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # Aggregate context vectors across time steps
        context = lstm_out.mean(dim=1)  # (batch_size, hidden_size * 2)
...
def train( model, dataloaders):
    ...
    for i in range(num_epochs):
        ...
        train_loss = train_lossi/step-0.5
